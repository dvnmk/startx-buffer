...
void setup()
{  
  pinMode(stromPin, OUTPUT);   digitalWrite(stromPin, LOW); // strom init y aus
  pinMode(ms, OUTPUT);         digitalWrite(ms, HIGH); // alle_ms 1/16
  Serial.begin(115200); 
  ...
  pinMode(enU, OUTPUT);     digitalWrite(enU, HIGH);  pinMode(enV, OUTPUT);     digitalWrite(enV, HIGH); 
  pinMode(enW, OUTPUT);     digitalWrite(enW, HIGH);  pinMode(enX, OUTPUT);     digitalWrite(enX, HIGH); 
...
void loop()
{
  ...
  if ( Serial.available()) {
    ...
    case '=' : //setSpeed eingentlich, not need.
